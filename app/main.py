...
def parse_adk_event(event):
    ...
    # Skip empty messages entirely unless they are tool calls
    if text_content.strip() == "" and len(tool_calls) == 0:
        return None
...
async def get_history(session_id: str):
    """Fetches the past chat transcript to restore on page reload."""
    try:
