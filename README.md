...
- **Demo Mode:** An optional frontend toggle switch ("faster, less costly"). When enabled, the `DynamicGemini` model wrapper overrides the `STRONG_MODEL` assignments and forcibly routes all heavy operations through the highly efficient `gemini-3.5-flash` model.
