...
* **Environment-Aware Tool Configuration:** When utilizing a server-side built-in tool (like `google_search`) alongside custom function calling, the Gemini Developer API requires `tool_config=types.ToolConfig(include_server_side_tool_invocations=True)`. However, Google Cloud Vertex AI explicitly rejects this parameter. To support both environments effortlessly, `app/agent.py` must conditionally inject this config only if `GEMINI_API_KEY` is detected in the environment variables.

## 6. Operational Maintenance
* **Cost Estimation & Price Lists:** The `TokenCounterPlugin` calculates accurate costs by tracking input/output tokens per model. The exact price values (`GEMINI_PRICING`) are hardcoded in `app/agent.py` for simplicity and performance. Every time a new model is added, or its prices change (including demo mode overrides), this list MUST be updated. 
* **Monthly Price Verification:** To prevent prices from silently decaying, `app/agent.py` includes a `LAST_PRICE_UPDATE` date constant. The integration tests enforce that this date is never older than 30 days, serving as an automatic reminder for developers to verify the pricing monthly.
* **Stabilizing Lite Models:** Small/fast models (like gemini-flash-lite) are highly susceptible to "instruction drift" when processing dense, unstructured prompts, often skipping critical formatting requirements (like outputting a `[STATUS]` tag). To guarantee deterministic behavior, always partition the prompt with rigid XML tags (e.g., `<RULES>`, `<FORMATTING>`) and enforce a mandatory `<THINKING>` block before the final output to force Chain-of-Thought reasoning.
