...
5. **Live Dialectical Debate UI (Phase 2/3)**: During the execution of the debate loop, the frontend must visually display the ongoing back-and-forth arguments between the Protagonist and the Antagonist, alongside a prominent badge showing the active lens. The user must be able to "read along" and observe the dialectical friction. The UI must explicitly filter out internal backend tags (like `[DECISION: END]`), and must prominently highlight any hallucinated citations removed by the Academic Integrity Auditors in a distinct red alert box.
6. **Visual Documentation**: Core concepts must strictly be visualized as Mermaid diagrams (`DIAGRAMS.md`) to enable rapid onboarding for junior developers.
